...
def cells(
    ...
    if os.path.exists(save_path):
        if backup_folder:
            backup_path = os.path.join(os.path.dirname(save_path), "_".join([backup_folder, time.strftime("%Y%m%d-%H%M%S")]))
            os.makedirs(backup_path, exist_ok=True)
            shutil.move(save_path, os.path.join(backup_path, os.path.basename(save_path)))
        else:
    ...
    if os.path.exists(table_path := save_path.replace(*table_suffix)):
        shutil.move(table_path, os.path.join(backup_path, os.path.basename(table_path)))
    ...
    if os.path.exists(yaml_path := save_path.replace(*params_suffix)):
        shutil.move(yaml_path, os.path.join(backup_path, os.path.basename(yaml_path)))
